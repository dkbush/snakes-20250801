...
class Game:
    ...
    def draw(self):
        ...
        time_surf = font.render(f"Time: {minutes:02d}:{seconds:02d}", True, BLUE)
        screen.blit(time_surf, (8, 50))
    ...
    def run(self):
        while True:
            ...
            if not self.game_over:
                ...
                if self.time_up:
                    over = font.render("TIME'S UP!", True, BLUE)
                else:
                ...
                score_text = score_font.render(f"Final Score: {self.score}", True, BLUE)
                ...
                sr = sub.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 90))
                screen.blit(sub, sr)
                pygame.display.flip()
            clock.tick(self.fps)
